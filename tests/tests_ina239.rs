...
#[test]
fn reset_test() {
    let expected_txns = [write_txn(0x00, 1 << 15), read_txn(0x05, &[0x00, 0x00])].concat();
    let mut spi = SpiMock::new(&expected_txns);
...
fn test_manufacture_id() {
    let mut spi = SpiMock::new(&[read_txn(0x3E, &[0x54, 0x49])].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_device_id() {
    let mut spi = SpiMock::new(&[read_txn(0x3F, &[0x23, 0x91])].concat());
    let mut ina = INA239::new(spi.clone());
    let did = ina.device_id().unwrap();
    assert_eq!(did, (0x239, 0x1));
    spi.done();
}
...
fn test_calibration() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x02, shunt_cal)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_configuration() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x00, confg_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_adc_configuration() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x01, exp_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_shunt_volt() {
    let mut spi = SpiMock::new(&[read_txn(0x04, &0x0020_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_bus_volt() {
    let mut spi = SpiMock::new(&[read_txn(0x05, &0x0010_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_die_temp() {
    let mut spi = SpiMock::new(&[read_txn(0x06, &0x0040_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_current() {
    let mut spi = SpiMock::new(&[read_txn(0x07, &0x0001_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
    ina.current().unwrap();
    spi.done();
}
...
fn test_shunt_overvolt() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x0C, exp_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_shunt_undervolt() {
    ...
    let exp_val = exp_shunt_volt(-0.04, true) as u16;
    let mut spi = SpiMock::new(&[write_txn(0x0D, exp_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_bus_overvolt() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x0E, exp_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_bus_undervolt() {
    ...
    let mut spi = SpiMock::new(&[write_txn(0x0F, exp_val)].concat());
    let mut ina = INA239::new(spi.clone());
...
fn test_temp_overlimit() {
    ...
    let expected_txns = [write_txn(0x10, neg_val), write_txn(0x10, pos_val)].concat();
...
fn test_power_overlimit() {
    ...
    let expected_txns = [write_txn(0x02, shunt_cal), write_txn(0x11, exp_val)].concat();
...
fn test_get_power_overlimit() {
    let mut spi = SpiMock::new(&[read_txn(0x11, &0xFFFF_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
...
#[test]
fn test_diag_flags() {
    let mut spi = SpiMock::new(&[read_txn(0xB, &0xFFFF_u16.to_be_bytes())].concat());
    let mut ina = INA239::new(spi.clone());
...
// ---- Helper functions ----
fn read_txn(reg: u8, bytes: &[u8]) -> [SpiTransaction<u8>; 3] {
...
fn write_txn(reg: u8, value: u16) -> [SpiTransaction<u8>; 3] {
